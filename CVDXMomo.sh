...
####==toml==####
    ...
    if [[ $NUM -eq 0 ]] ;then
        ...
        sed -e "s|m@_@m|ss${NUM}|" \CR31="[CJODMomo IENEMomo VENXMomo]"
            -i "${WD}"/Iosevka/private-build-plans.toml
    fi
...
cp "${WD}"/Cascadia-NOMOD/*.ttf -t "${WD}"
#    cp "${WD}"/Iosevka-OB/*.ttf  -t "${WD}"

#    for i in {Bold,BoldItalic,Italic,Regular} ;do
#        /tmp/scr/ios2cas.pe "${WD}"/IOS-${i}.ttf   "${WD}"/CASOB-${i}.ttf
#        /tmp/scr/ios2cas.pe "${WD}"/IOSNL-${i}.ttf "${WD}"/CASNLOB-${i}.ttf
#    done
#    rm "${WD}"/IOS*.ttf
#    rm "${WD}"/CAS*.ttf
#    for i in {Bold,BoldItalic,Italic,Regular} ;do
...
#    done
for i in {Bold,BoldItalic,Italic,Regular} ;do
    mv "${WD}"/CAS-${i}.ttf   "${WD}"/JetBrainsMono-${i}.ttf
    mv "${WD}"/CASNL-${i}.ttf "${WD}"/JetBrainsMonoNL-${i}.ttf
done
...
sed -e "s|\[JetBrains Mono\]|${CR11}|g" \
    -e "s|^.*2020\sThe.*$|${CR12}\n\n${CR21}\n${CR22}|g" \
    -e "s|Otawara|Otawara\n\n${CR31}\n${CR32}\n${CR33}|g" \
    -i "${WD}"/UDEV/copyright.sh
sed -e 's|VERSION=.*$|VERSION="1.0.0"|g' \
    -e 's|UDEVGothic|CVDXMomo|g' \
    -e 's|UDEV\sGothic|CVDXMomo|g' \
    -e 's|UDEV\sGothic\s|CVDXMomo|g' \
    -i "${WD}"/UDEV/make.sh
cd "${WD}"/UDEV
./make.sh
mv    "${WD}"/UDEV/build         -T "${DIR}"/CVDXMomo
mv    "${WD}"/fontname.py        -t "${DIR}"/CVDXMomo
cp -r "${DIR}"/DLsources/LICENSE -t "${DIR}"/CVDXMomo
