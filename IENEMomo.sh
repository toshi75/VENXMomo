...
sed -e 's|VERSION=.*$|VERSION="1.0.0"|g' \
    -e 's|UDEVGothic|IENEMomo|g' \
    -e 's|UDEV\sGothic|IENEMomo|g' \
    -e 's|UDEV\sGothic\s|IENEMomo|g' \
    -i "${WD}"/UDEV/make.sh
cd "${WD}"/UDEV
./make.sh
mv    "${WD}"/UDEV/build         -T "${DIR}"/IENEMomo
mv    "${WD}"/fontname.py        -t "${DIR}"/IENEMomo
cp -r "${DIR}"/DLsources/LICENSE -t "${DIR}"/IENEMomo
