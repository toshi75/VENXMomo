...
sed -e 's|VERSION=.*$|VERSION="1.0.0"|g' \
    -e 's|UDEVGothic|VENXMomo|g' \
    -e 's|UDEV\sGothic|VENXMomo|g' \
    -e 's|UDEV\sGothic\s|VENXMomo|g' \
    -i "${WD}"/UDEV/make.sh
cd "${WD}"/UDEV
./make.sh
mv    "${WD}"/UDEV/build         -T "${DIR}"/VENXMomo
mv    "${WD}"/fontname.py        -t "${DIR}"/VENXMomo
cp -r "${DIR}"/DLsources/LICENSE -t "${DIR}"/VENXMomo
